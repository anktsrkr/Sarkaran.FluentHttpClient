...
namespace Sarkaran.Http.Client.Internal
{
    ...
    public sealed class Request : IRequest
    {
        ...
        public IRequest WithArgument<T>(T argument)
        {
            if (argument == null)
                return this;
            ...
            var args = argument
                         .GetType()
                         .GetProperties()
                         .ToDictionary(p => p.Name, p => p.GetValue(argument))
                         .Select(pair => new KeyValuePair<string, object>(pair.Key, pair.Value)).ToArray();

            this.Message.RequestUri = this.Message.RequestUri.WithArguments(this.Options.IgnoreNullArguments ??
                false, args);
            return this;
        }
